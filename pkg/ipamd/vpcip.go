...
func (s *ipamServer) appendPool(size int) {
	...
	for _, ip := range vpcIps {
		...
		klog.Infof("add ip %s to pool", ip.Ip)
	}
}
...
func (s *ipamServer) runGC() {
	ips, err := s.pool.List()
	if err != nil {
		klog.Errorf("failed to list ip, error: %v", err)
		return
	}
	...
	if err != nil {
		klog.Warningf("failed to init gc, error: %v, we will use an empty one", err)
	...
	if err != nil {
		klog.Errorf("failed to run gc, error: %v", err)
		return
	}
}
